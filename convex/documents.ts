...
export const archived = mutation({
    ...
    handler:async(ctx,args) =>{
        ...
        const recusiveArchive = async (documentId:Id<"documents">)=>{
...
export const resotre = mutation({
    ...
    handler:async(ctx,args)=>{
        ...
        const recusiveResotre =  async (documentId:Id<"documents">)=>{
            const children = await ctx.db.query("documents").withIndex("by_user_parent",(q)=>(
                q.eq("userId",userId)
                .eq("parentDocument",documentId)
            ))
            ...
            for(const child of children){
                ...
                await recusiveResotre(child._id);
            }
        }
...
export const getById = query({
    ...
    handler:async(ctx,args)=>{
        ...
        if(!identity) {
            throw new Error ("Not authothicated")
        }
        const userId  = identity.subject
